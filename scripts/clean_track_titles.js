...
function processFile(filePath) {
    try {
        ...
        if (!data.tracks || !Array.isArray(data.tracks)) {
            console.log(`Skipping ${path.basename(filePath)} - no tracks array found`);
            return;
        }
...
function main() {
    ...
    console.log(`Files modified: ${stats.changes.reduce((acc, change) => {
    ...
    if (stats.timeIssues.length === 0) {
        ...
        stats.timeIssues.forEach(issue => {
            ...
            console.log(`  Track ${issue.trackIndex}: "${issue.currentTrack}" (${issue.currentTime})`);
            console.log(`  Previous: "${issue.previousTrack}" (${issue.previousTime})`);
            console.log('');
        });
    }
