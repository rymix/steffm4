...
if (report.timeIssues.length > 0) {
    ...
    report.timeIssues.forEach((issue, index) => {
        ...
        console.log(`   Time: ${issue.startTime} (previous: ${issue.previousTime})`);
        console.log('');
    });
} else {
    console.log('\n=== START TIME ISSUES ===');
...
fs.writeFileSync('track_cleaning_report.json', JSON.stringify(detailedReport, null, 2));
console.log(`Detailed report written to track_cleaning_report.json (${detailedReport.allChanges.length} changes listed)`);