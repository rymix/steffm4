...
export const StyledJupiterButtonWrapper = styled.div<StyledJupiterButtonWrapperProps>`
  ...
  width: ${(props) => (props.$size === "huge" ? "100%" : "128px")};
  width: 54px;

  @media screen and (orientation: portrait) and (max-width: 440px) {
    width: ${(props) => (props.$size === "huge" ? "100%" : "44px")};
  }
`;
...
export const StyledJupiterButton = styled.button<StyledJupiterButtonProps>`
  ...
  cursor: pointer;
  height: ${(props) => (props.$size === "huge" ? "144px" : "72px")};
  width: ${(props) => (props.$size === "huge" ? "84px" : "42px")};
  overflow: hidden;
  position: relative;
  transition:
...
export const StyledJupiterLed = styled.div<StyledJupiterLedProps>`
  ...
  border-radius: 50%;
  height: ${(props) => (props.$size === "huge" ? "24px" : "12px")};
  left: ${(props) => (props.$size === "huge" ? "26px" : "12px")};
  position: absolute;
  top: ${(props) => (props.$size === "huge" ? "8px" : "4px")};
  transform: ${(props) => (props.$down ? "scale(0.9)" : "scale(1)")};
  transition:
    background-color 0.1s ease-in-out,
    box-shadow 0.1s ease-in-out,
    transform 0.1s ease-in-out;
  width: ${(props) => (props.$size === "huge" ? "24px" : "12px")};
