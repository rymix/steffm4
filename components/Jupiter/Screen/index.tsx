...
const JupiterScreen: React.FC = () => {
  ...
  const startScreenInterval = useCallback(
    (
      message: string,
      interval: number,
      messageType: "holding" | "track" | "temporary",
    ) => {
  ...
  useEffect(() => {
    if (
      holdingMessage !== previousHoldingMessage &&
      currentMessageType === "holding"
    ) {
      startHoldingMessage();
    }
  ...
  useEffect(() => {
    const sliced = screenMessage.slice(
      screenPosition,
      screenPosition + displayLength,
    );
